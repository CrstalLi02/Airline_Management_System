...
export default function FlightsInTheAir() {
  ...
  useEffect(() => {
    document.title = 'Flights in the Air';
  ...
  const headers = ['From', 'To', '# Flights', 'Flight List', 'Earliest', 'Latest', 'Airplane List'];

  return (
    <PageTemplate title="Flights in the Air">
      <div style={{ maxWidth: 800, margin: '40px auto', padding: 16 }}>
        <h2 style={{ marginBottom: 16 }}>Flights in the Air</h2>
        {loading && <p style={{ textAlign: 'center' }}>Loading...</p>}
        {error && <p style={{ textAlign: 'center', color: 'red' }}>Error: {error}</p>}
        {!loading && !error && rows.length === 0 && (
          <p style={{ textAlign: 'center' }}>No flights in the air.</p>
        )}
        {!loading && !error && rows.length > 0 && (
