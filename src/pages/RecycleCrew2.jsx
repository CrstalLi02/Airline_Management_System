...
const RecycleCrew = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Failed to recycle crew. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageTemplate 
      title="Recycle Crew" 
      description="Reassign flight crew to different flights"
    >
      {success && (
        <div className="mb-6 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded flex items-center">
          ...
          <span>Flight crew recycled successfully!</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4 flex justify-between">
          ...
          >
            {loading ? 'Processing...' : 'Recycle Crew'}
          </button>
        </div>
      </form>
    </PageTemplate>
